...
private:
    ...
    void set_position();
    //Helper function: update positions in linkedlist to be in order from 1 to N, where N is the number of element in linkedlist.
    void swap(int& m, int& n);
    //Helper function: swap two values
    ListNodePtr copy_node(ListNodePtr);
    //Helper function
    //Precondition: Start with a pointer that points to a node.
    //Postcondition: return the copy version of the argument node.
